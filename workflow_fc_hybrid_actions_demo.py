...
功能：
- 通用业务工作流编排（支持上万业务动作的扩展）
- 多轮 function-calling 结构规划（search + add_node + add_edge + finalize）
- 需求覆盖校验 + 结构增量改进
- 参数补全 + 静态校验（含数组元素字段检查）
- 自修复编排器（根据错误列表再让 LLM 自动修 workflow，多轮）
- result_of.<node_id> 数据流 + 绑定 DSL（__from__）+ composable pipeline DSL
- 保证 edges 覆盖所有节点（ensure_edges_connectivity）
- last_good_workflow 保护，避免 LLM 改坏顶层结构导致 KeyError('nodes')
...
# ===================== 4. HybridActionSearchService：BM25 + 向量融合 =====================
        ...
        while queue:
            ...
            if ntype == "action" and action_id:
                ...
                if not action:
                    ...
                    if action_id == "hr.get_today_temperatures.v1":
                        result = {
                            "data": [
                                {"employee_id": "E001", "temperature": 36.8},
                                {"employee_id": "E002", "temperature": 38.2},
                                {"employee_id": "E003", "temperature": 37.0},
                            ]
                        }

                    elif action_id == "hr.update_employee_health_profile.v1":
                        result = {
                            "data": [
                                {
                                    "employee_id": resolved_params.get("employee_id", "unknown"),
                                    "last_temperature": resolved_params.get("last_temperature", 36.5),
                                    "status": resolved_params.get("status", "normal"),
                                }
                            ]
                        }
...
def main():
    ...
    for a in BUSINESS_ACTIONS:
        ...
        vec_client.upsert(a["action_id"], emb)

    hybrid_searcher = HybridActionSearchService(
        es=fake_es,
