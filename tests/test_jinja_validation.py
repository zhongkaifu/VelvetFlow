...
def test_malformed_ternary_expression_is_repaired():
    workflow = _workflow(
        {
            "status": "abnormal' if (loop.item.temperature is not none and (loop.item.temperature | float) > 38) else 'normal",
        }
    )
    ...
    assert summary["applied"] is True
    assert (
        normalized["nodes"][0]["params"]["status"]
        == "{{ 'abnormal' if (loop.item.temperature is not none and (loop.item.temperature | float) > 38) else 'normal' }}"
    )
