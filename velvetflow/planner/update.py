...
def update_workflow_with_llm(
    ...
    system_prompt = (
        ...
        "  condition 节点的 params 仅包含 expression（单个返回布尔值的 Jinja 表达式）以及 true_to_node/false_to_node（字符串或 null）。\n"
        ...
        "3) condition/loop/parallel 节点需要补齐必填字段：condition 必须提供 true_to_node/false_to_node（指向真实存在的下游节点，或设置为 null 表示该分支结束）且 params.expression 为布尔 Jinja 表达式；loop 需包含 iter/body_subgraph/exports 等，引用路径必须合法。\n"
