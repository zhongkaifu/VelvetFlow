...
def update_workflow_with_llm(
    ...
    system_prompt = (
        ...
        "  引用 loop.exports 结构时必须指向 exports 内部的字段或子结构，不能只写 result_of.<loop_id>.exports。\n"
        ...
        "5) edges 将由系统基于节点引用自动推导，不需要手动增删；入口节点通常是 type=start，出口节点通常是 type=end。\n"
        "6) 所有绑定表达式 (__from__/__agg__) 必须与上游 action 的 output_schema 或 loop.exports 对齐，避免访问不存在的字段；__agg__ 必须使用 identity/count/count_if/join/format_join/filter_map/pipeline 之一，其他取值视为非法并会在校验报告中列出。需要条件过滤时请把 __agg__.condition 或 pipeline.steps[].condition 写成 Jinja 表达式，直接引用 item/value/field 等上下文变量，严禁再写自然语言条件。\n"
