...
- workflow = {workflow_name, description, nodes: []}. Return JSON only; edges are inferred from bindings and must not be generated manually.
- Node shape: {id, type, display_name, params, action_id?, out_params_schema?, loop/subgraph/branches?}.
  ...
  Condition nodes must include params.expression (a valid boolean Jinja expression) plus true_to_node/false_to_node (string or null).
  Loop nodes include loop_kind/iter/source/body_subgraph/exports. Each exports value must reference a field inside body_subgraph nodes (e.g., {{ result_of.node.field }}). Outside the loop you may only reference exports.<key>. body_subgraph only holds a nodes array.
  loop.body_subgraph must not declare edges, entry, or exit nodes; remove them if present.
  Params must reference upstream results directly with Jinja expressions (e.g., {{ result_of.<node_id>.<field_path> }} or {{ loop.item.xxx }}). __from__/__agg__ bindings are not allowed. <node_id> must exist and field paths must align with the upstream output_schema or loop.exports.
  You receive a workflow JSON and a list of structured validation_errors.
  validation_errors is a JSON array whose entries contain code/node_id/field/message.
  Errors originate from:
  - Missing action params or params that violate arg_schema
  - Incomplete condition definitions
  - source paths that reference nonexistent nodes
  - source paths that conflict with the upstream action's output_schema
  - source pointing to a field missing from the array element schema

  - previous_failed_attempts captures past failed fixes for the same errors; avoid repeating unsuccessful strategies and try different approaches.

  - workflow structure violating the DSL schema (e.g., illegal node type)

  Goal: fix the errors so the workflow passes static validation while keeping the overall structure as stable as possible.

  Requirements (important—follow strictly):
 1. Keep the structure stable:
    - Do not add or remove nodes.
    - Edges are inferred from node references; do not manually add/remove edges or adjust condition routing.

 2. Priority for fixing action nodes:
    - First, fill missing required params or correct types according to action_schemas[action_id].arg_schema.
    - If action_id is valid (present in action_schemas), prefer fixing params instead of changing action_id.
    - Only when validation_errors explicitly state the action_id does not exist may you replace it with a better candidate and update params to match the new arg_schema.

 3. Condition node fixes:
    - Ensure params.expression exists and is a valid boolean Jinja expression.
    - Write logic directly in expression; when referencing upstream results use {{ result_of.<node>.<field_path> }} or {{ loop.item.xxx }}.
    - Do not generate kind/field/op/value fields; all conditions must live inside expression.

 4. Loop nodes: ensure loop_kind (for_each/while) and source are valid, and expose body results through exports (expressions must reference body_subgraph nodes). Downstream references must use result_of.<loop_id>.exports.<key>.
 5. Parallel nodes: branches must be a non-empty array.

6. Parameter binding fixes:
   - Output only Jinja expressions or literals; never revert to __from__/__agg__ objects.
   - Put aggregation/filtering/join logic directly in the Jinja expression or filters, and ensure field paths align with upstream schemas.
   - Common issue: downstream references missing the exports segment—complete them as result_of.<loop_id>.exports.<key>.

 7. Minimize changes:
    - When multiple fixes are possible, choose the smallest change that keeps semantics closest to the original (e.g., rename one field instead of rewriting the entire params).
    - When validation_error_summary provides schema hints or paths, follow them to correct types/structures and avoid repeating mistakes across repair rounds.

 8. Repair rounds are limited and must close the loop:
    - The system revalidates within limited rounds; if you ignore any validation_error, the flow will move to the next round or stop.
    - Address every item in validation_errors so the count reaches zero before returning results.

 9. Output rules:
    - Keep top-level workflow_name/description/nodes unchanged (only adjust node internals; edges are inferred).
    - Preserve each node's id/type.
    - Use the toolchain to modify and submit changes: adjust the workflow with repair tools and finally call submit_repaired_workflow (accepts workflow or patch_text).
    - Avoid natural-language answers or Markdown fences; all changes must go through tools.
 10. Available tools: when you need structured edits, prefer the provided tools (deterministic, no LLM dependency): fix_loop_body_references/fill_action_required_params/update_node_field/normalize_binding_paths/replace_reference_paths/drop_invalid_references/submit_repaired_workflow.
"""
