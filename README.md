...
## 快速导读
- **给投资人**：VelvetFlow 用更少的人工投入，将“文字需求→可执行流程”自动化，并内置安全审计与自修复，缩短 PoC 周期，提升交付确定性。
- **给 CTO**：系统由混合检索、两阶段 LLM 规划、自动防御式校验/修复、可视化和模拟执行器组成，可通过标准化 DSL 复用到不同业务域。
- **给初级工程师**：本文档从运行指令到 DSL 细节、再到自动修复和执行流水线逐步展开，尽量保证“只读 README 也能复刻一份”。
